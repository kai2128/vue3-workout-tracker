    ...
    <div v-if="dataLoaded">
      <div class="flex flex-col items-center rounded-md shadow-md bg-light-grey relative p-8">
        <div v-if="user" class="flex absolute left-2 top-2 gap-x-2 text-at-light-green text-lg">
          <ph:trash-duotone
            class="cursor-pointer hover:text-at-green duration-200"
        ...
        <input
          v-if="editMode"
          class="mt-4 text-3xl text-at-light-green p-2 text-gray-500 focus:outline-none"
          v-model="data.name"
        />
        ...
        <div class="flex flex-col gap-y-4 w-full">
          ...
          >
            ...
            <ph:trash-simple
              ...
              class="text-sm absolute cursor-pointer text-at-light-green -left-5 top-0.25 duration-200 hover:text-at-green"
            ></ph:trash-simple>
          </div>
          ...
          >
            <div class="item flex-2 md:w-1/3">
              <label for>Cardio Type</label>
              <select v-if="editMode" type="text" v-model="item.cardioType">
                <option value>Please select</option>
                <option value="run">Run</option>
                <option value="walk">Walk</option>
              </select>
              <input v-if="editMode" type="text" v-model="item.cardioType" />
              <p v-else>{{ item.cardioType }}</p>
            </div>
            <div class="item flex-1">
              <label for>Distance</label>
              <input v-if="editMode" type="text" v-model="item.distance" />
              <p v-else>{{ item.distance }}</p>
            </div>
            <div class="item flex-1">
              <label for>Duration</label>
              <input v-if="editMode" type="text" v-model="item.duration" />
              <p v-else>{{ item.duration }}</p>
            </div>
            <div class="item flex-1">
              <label for>Pace</label>
              <input v-if="editMode" type="text" v-model="item.pace" />
...
const getData = async () => {
  try {
    ...
    Object.assign(initialData, workouts)
    dataLoaded.value = true;

    watch(data, () => {
      if(JSON.stringify(data.value) != JSON.stringify(initialData))
        setMsg('Changes not saved', MessageType.INFO);
    })
  } catch (error) {
...
const deleteExercise = (index) => {
  ...
  setMsg('Must have more than 1 exercise', MessageType.ERROR);
};
