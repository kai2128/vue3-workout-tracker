...
<template>
  <div class="container mt-10 mx-auto px-4" v-if="dataLoaded">
    ...
    <div v-else class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        <router-link class="p-8 flex flex-col items-center bg-light-grey cursor-pointer rounded-lg hover:shadow-md duration-200 animate-ease-[cubic-bezier(0.25,0.1,0.25,1)]" :to="{name : 'ViewWorkout'}" v-for="(workout, index) in data" :key="workout.id">
