    ...
    <div v-if="dataLoaded">
      <div class="flex flex-col items-center rounded-md shadow-md bg-light-grey relative p-8">
        <div v-if="user" class="flex absolute left-2 top-2 gap-x-2 text-at-light-green text-lg">
          <ph:trash-duotone class="cursor-pointer hover:text-at-green" @click="deleteWorkouts"></ph:trash-duotone>
          <ph:pencil-line-duotone class="cursor-pointer hover:text-at-green" @click="toggleEditMode"></ph:pencil-line-duotone>
        </div>
        ...
        <h1 class="mt-4 text-3xl text-at-light-green">{{data.name}}</h1>
      </div>
      ...
      <div class="mt-10 p-8 rounded-md flex flex-col items-center bg-light-grey shadow-md">
        <div v-if="data.type == 'strength'" class="flex flex-col gap-y-4 w-full">
          ...
          >
            ...
            <ph:trash-simple class="text-sm absolute cursor-pointer text-at-light-green -left-5 top-0.25 duration-200 hover:text-at-green"></ph:trash-simple>
          </div>
          <button
            v-if="editMode"
            type="button"
