<template>
    <div class="max-w-screen-sm mx-auto px-4 py-10">
        <div v-if="dataLoaded">
            ...
            <div v-if="data.type == 'strength'" class="flex flex-col items-center rounded-md shadow-md bg-light-grey relative p-8">
              <div class="flex" v-for="(item, index) in data.exercises" :key="item.id">
                <div class="flex flex-col md:w-1/3">
                  <label for="" class="text-at-light-green text-xs mb-1">Exercise</label>
                  <input v-if="editMode" type="text" v-model="item.exercise" class="p-2 focus:outline-none w-full">
                  <p v-else>{{item.exercise}}</p>
                </div>

              </div>
            </div>
            <div v-else class="flex flex-col items-center rounded-md shadow-md bg-light-grey relative p-8">
                  <div class="flex" v-for="(item, index) in data.exercises" :key="item.id">
