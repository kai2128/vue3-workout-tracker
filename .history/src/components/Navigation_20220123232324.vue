        ...
        <header class="bg-at-light-green text-white">
            <nav class="container mx-auto py-5 px-4 flex flex-col gap-4 items-center" sm="flex-row">
                ...
                <ul class="flex flex-1 justify-end gap-x-10">
                    ...
                    <router-link v-if="loggedIn" class="cursor-pointer" to="/">Create</router-link>
                    <router-link v-if="loggedOut" class="cursor-pointer" :to="{name: 'Login'}">Login</router-link>
                    <li v-if="loggedIn" class="cursor-pointer" @click="logout">Logout</li>
                </ul>
            </nav>
        </header>
</template>
...
const {loggedOut, loggedIn} = useUserStore();
const logout = async () => {
