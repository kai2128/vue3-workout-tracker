...
const getData = async () => {
  try {
    nProgress.start();
    let workouts, error = null;
    if(user_id){
      const {data, error: err} = await supabase.from('workouts').select('*').eq('user_id', user_id);
      workouts = data
      error = err;
    }else{
      const {data, error: err} = await supabase.from('workouts').select('*');
      workouts = data
      error = err;
    }
